...
ただ懸念点もある。
- Vault全体はSubmoduleにしたくないので特定のディレクトリのみ公開したい条件がクリアできない。
- dispatchの作成とかも複雑になるし困るなぁ。あまりやりたくない。

GPTと相談して以下のシーケンスにしてみた。
PATを登録すると vaultのレポからquartzのレポにPushできるらしいのでそれがシンプルにできそう。
...
privateレポジトリにPushするのでPATの発行と設定が必要。
withを使用すると仮想環境が `obsidian-blog` ディレクトリにquartz相当のrepoを配置してくれる。
rsyncでそこに copy して差分Pushする仕組み。
